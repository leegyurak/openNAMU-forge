...
async def main_setting_404_page():
    ...
    if flask.request.method == 'POST':
        ...
        return redirect('/setting/404_page')
    else:
        ...
        data_html += (
            '<form method="post">' +
                '<textarea class="opennamu_forge_textarea_500 __ON_TEXTAREA__" name="data" placeholder="' + lang['data']['enter_html'] + '">' + html.escape(form_data) + '</textarea>' +
                '<hr class="main_hr">' +
                '<button class="__ON_BUTTON__" id="opennamu_forge_save_button" type="submit">' + lang['data']['save'] + '</button>' +
            '</form>'
        )
