...
async def edit_editor(conn, ip, data_main = '', do_type = 'edit', addon = '', name = ''):
    ...
    # 문법 선택창
    if do_type == 'edit':
        monaco_editor_top += view_set_markup(conn, document_name = name, addon = 'id="opennamu_forge_editor_markup" onclick="opennamu_forge_do_sync_monaco_markup();"')
    else:
        monaco_editor_top += view_set_markup(conn, addon = 'id="opennamu_forge_editor_markup" onclick="opennamu_forge_do_sync_monaco_markup();"', disable = 'disabled')
    ...
    with get_db_connect() as conn:
        ...
        if edit_repeat == 'post':
            ...
            if edit_timeout_value != '' and platform.system() in ('Linux', 'Darwin'):
                timeout = await edit_timeout(conn, name, content, int(edit_timeout_value))
            else:
                timeout = 0
