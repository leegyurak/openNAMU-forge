...
- [ ] `opennamu_forge/presentation/shared/func.py` 의 route 의존성을 줄인다.
...
- [ ] MySQL/PostgreSQL integration test의 skip 조건을 점검한다.
  - CI에서 서비스 컨테이너로 실제 실행할 수 있으면 skip 대신 실행하도록 조정한다.
  - 로컬에서는 환경변수로 명시 opt-in 할 수 있게 유지한다.
- [ ] SQLite/MySQL/PostgreSQL 모두 Alembic migration 후 기본 route 동작을 검증한다.
- [ ] Prometheus `/metrics` 노출과 비활성화 설정을 integration test로 고정한다.

## P2 - Runtime 구조 정리

- [ ] `runtime_app.py` 를 더 얇게 만든다.
  - Flask app composition, startup task 실행, signal wiring, serve entrypoint를 더 명확히 나눈다.
- [ ] GopenNAMU helper 의존 범위를 계속 줄인다.
  - Python route가 직접 처리할 수 있는 기능과 helper binary 위임이 필요한 기능을 분리해서 기록한다.
  - 제거가 목표가 되는 경우 별도 migration plan을 작성한다.
- [ ] process/scheduler 테스트 공백을 줄인다.
  - 낮은 coverage 모듈부터 분기 테스트를 보강한다.

## P2 - Frontend/Template 부채

- [ ] `views/ringo`의 HTML/CSS/JS에 남은 namespace와 레거시 문구를 재검색한다.
- [ ] 반응형 요구사항을 route/template 변경마다 검증한다.
  - desktop/tablet/mobile viewport smoke test를 유지한다.
- [ ] theme color 환경변수 적용 범위를 문서화한다.
