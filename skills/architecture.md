...
- SQLAlchemy/SQLModel query spec/composer는 infrastructure에 둡니다.
...
4. 동적 repository filter와 OR query fragment는 SQL tautology나 repository-local `or_`가 아니라 infrastructure query spec/composer로 분리합니다.
5. route registry와 runtime composition은 한곳에서만 연결합니다.
6. legacy 경로 제거 시 새 경로로 참조가 모두 옮겨졌는지 `rg`로 확인합니다.
7. 문서와 agent 규칙을 새 구조에 맞게 갱신합니다.
...
- repository optional filter를 `or_(literal(not flag), condition)` 같은 SQL tautology로 표현하지 않습니다.
- repository method 본문에서 직접 `or_(...)`로 query fragment를 조립하지 않습니다.
